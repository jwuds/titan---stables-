...
/**
 * CRITICAL: Admin route patterns to exclude from schema markup
 */
...
export const ArticleSchema = ({ article }) => {
  ...
  // CRITICAL: Do not generate schema for admin routes
 ...
 * CRITICAL: Filters out admin routes from breadcrumb list
...
export const BreadcrumbSchema = ({ items }) => {
  ...
  // CRITICAL: Filter out admin routes from breadcrumb items
  const publicItems = items.filter(item => !isAdminRoute(item.path));
 ...
 * CRITICAL: Only generates schema for public locations, not admin routes
...
export const generateLocalBusinessSchema = (location) => {
  ...
  // CRITICAL: Do not generate schema if location slug contains admin
  if (location.slug && isAdminRoute(`/locations/${location.slug}`)) return null;
